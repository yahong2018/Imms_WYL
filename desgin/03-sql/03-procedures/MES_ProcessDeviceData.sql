...
begin
    ...
    begin tran;
	    ...
	    if not exists(
		   select * from mes_workstation_product_summary
		     where order_no = @WorkorderNo
			   and workstation_code = @WorkstationCode
		)
		   insert into mes_workstation_product_summary(order_no,part_no,line_no,workstation_code,qty_good,qty_bad)
		                    values(@WorkorderNo,@PartNo,@LineNo,@WorkstationCode,@QtyGood,@QtyBad);
        else
		   update mes_workstation_product_summary
